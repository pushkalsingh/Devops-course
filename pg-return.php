...
$transaction_status = mysqli_real_escape_string($dbhandle,$_GET['t']);


$curdate = date('Y-m-d H:i:s');

mysqli_query($dbhandle, "insert into trans_log (orderid, userid, datetime, description, entryby, status) values ('$orderid', '$userid', '$curdate', 'Returned from Payment Gateway', '$customer_name', 'unverified')");
...
else
{
	...
	if($numregdata>0)
	{
		...
		while($fetcheckregid=mysqli_fetch_array($resregdata))
		{
			...
			$delivery_charge = $fetcheckregid["delivery_charges"];
		...
		if(!empty($transaction_status))
		{
			...
			{
				$trans_amount = $total_amt-10;
				$trans_status = "Failed";
			}
			else
			{
				$trans_amount = $total_amt;
				$trans_status = "CHARGED";
			}
			...
			$customerId = $userid;
			$hash = "ABCD";
			$customerEmail = "team@insidesoftwares.com";
			$customerPhone = "9997411884";
			$currency = "rupees";
			$pgorderid = $orderid;
			...
			else
			{
				...
				if(($trans_status=='CHARGED') && ($orderid==$pg_orderId) && ($trans_amount>=$total_amt))
				{
					...
					<tr>
                        ...
                        <tr>
							<td>
								<img src='https://websoftwala.com/testarea/greenlifebazaar.in/images/logo/greenlifebazaar-logo-green.png' alt='' style=';margin-bottom: 20px;width:250px;'>
                            </td>
                        </tr>
                        <tr>
                            <td>
								<img src='https://websoftwala.com/testarea/greenlifebazaar.in/images/success.png' alt=''>
					...
					if(($nums = mysqli_num_rows($tmpcart_handler))>0)
					{
					 ...
					 while($tmpcart_rows = mysqli_fetch_array($tmpcart_handler))
					 {
						...
						$delivery_charge = $tmpcart_rows["delivery_charges"];
						...
						$prodrows = mysqli_fetch_array(mysqli_query($dbhandle, "select product_name, image_thumb, shortdesc from product where prodid='$prodid'"));
						$productname = ucwords($prodrows["product_name"]);
						$thumb_image = $prodrows["image_thumb"];
						$shortdesc = ucwords($prodrows["shortdesc"]);
						$prod_image = "https://websoftwala.com/testarea/greenlifebazaar.in/productimage/$prodid/$thumb_image";
			...
			$bodyemail.="<tr>
					...
					<tbody>
						<tr>
							...
							<td width='57' height='25' class='user-info'><img src='https://websoftwala.com/testarea/greenlifebazaar.in/images/space.jpg' alt='' height='25' width='57'></td>-->
        ...
        <table class='main-bg-light text-center top-0'  align='center' border='0' cellpadding='0' cellspacing='0' width='100%'>
			<tr>
				...
				<table border='0' cellpadding='0' cellspacing='0' class='footer-social-icon' align='center' class='text-center' style='margin-top:20px;'>
					...
					<tr>
						<td>
							<a href='#'><img src='https://websoftwala.com/testarea/greenlifebazaar.in/images/facebook.png' alt='' style='width:40px;'></a>
						</td>
						<td>
							<a href='#'><img src='https://websoftwala.com/testarea/greenlifebazaar.in/images/instagram.png' alt='' style='width:40px;'></a>
						</td>
						<!--<td>
							<a href='#'><img src='https://websoftwala.com/testarea/greenlifebazaar.in/images/youtube.png' alt='' style='width:40px;'></a>
						</td>-->
					</tr> 
					<tr>
				...
				<table  border='0' cellpadding='0' cellspacing='0' width='100%' style='margin: 20px auto 0;' >
					<tr>
						<td>
							<p style='font-size:13px; margin:0;'>2019  &copy; Copyright Greenlifebazaar Design &amp; Developed by 
					...
					$mail = new PHPMailer;
					$mail->From = 'info@greenlifebazaar.com';
					$mail->FromName = 'Greenlifebazaar';
					$mail->addAddress($customer_mail, $customer_name); 
					$mail->WordWrap = 50;      
					$mail->isHTML(true);                                  // Set email format to HTML

					$mail->Subject = 'Greenlifebazaar - Order Received';
					$mail->Body    = $bodyemail;
					...
					header("Location: success?r=$enc_regid&id=".base64_encode($orderid));
					}
